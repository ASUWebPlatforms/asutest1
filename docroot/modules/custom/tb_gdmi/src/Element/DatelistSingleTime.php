 ...
 *
 * @FormElement("datelist_single_time")
 */
class DatelistSingleTime extends DateElementBase {
  ...
  public function getInfo() {
    ...
    return [
      ...
      '#date_part_order' => ['year', 'month', 'day', 'time'],
      '#date_year_range' => '2023:2050',
      '#date_time_type' => '12',
      '#date_increment' => 30,
      '#date_date_callbacks' => [],
  ...
  public static function processDatelist(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    $element['#tree'] = TRUE;
    $element['#attributes']['class'][] = 'datelist-single-time'; 
    ...
    foreach ($order as $part) {
      switch ($part) {
        ...
        case 'month':
          $options = $date_helper->monthNames($element['#required']);
          $format = 'n';
          $title = t('Month');
          break;
        ...
        case 'time':
          $format = $element['#date_time_type'] === '24' ? 'H:i' : 'g:i A';
          $options = static::generateTimeOptions($element['#date_increment'], $element['#date_time_type']);
          $title = t('Time');
        break;
   ...
   * @param int $hoursFormat
   *   The hours format: '12' or '24'.
   *
   * @return array
   *   An array of time options.
   */
  public static function generateTimeOptions($increment = 15, $hoursFormat = 12) {
    $options = [];
    $selected_format = ($hoursFormat == 24) ? 'H:i' : 'g:i A';
    $other_format = ($hoursFormat == 24) ? 'g:i A' : 'H:i';

    $start_time = strtotime('00:00');
    $end_time = strtotime('23:59');

    while ($start_time <= $end_time) {
        $options[date($selected_format, $start_time)] = date($selected_format, $start_time) . ' (' . date($other_format, $start_time) . ')';
        $start_time += $increment * 60; // Increment in seconds
    }
