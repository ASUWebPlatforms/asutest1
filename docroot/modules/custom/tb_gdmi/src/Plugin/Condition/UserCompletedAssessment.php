...
/**
 * Provides a 'User Completed Assessment' condition.
 *
 * @Condition(
 *   id = "user_completed_assessment",
 *   label = @Translation("GDMI User has a completed assessment (results available)"),
 * )
 */
class UserCompletedAssessment extends ConditionPluginBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Creates a new UserCompletedAssessment instance.
   *
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['show'] = [
        '#title' => $this->t('Display only if the user has a completed assessment'),
        '#type' => 'checkbox',
        // Use whatever value is stored in cofiguration as the default.
        '#default_value' => $this->configuration['show'],
        '#description' => $this->t('When this box is checked, this block will only be shown if the user has a completed assessment, When negated, the block won\'t be shown if the user has a completed assessment.'),
    ];
  ...
  public function summary() {
    if ($this->configuration['show']) {
      ...
      if ($this->isNegated()) {
        ...
        return $this->t('Won\'t be shown if the user has a completed assessment');
      }
      else {
        // The condition is enabled.
        return $this->t('Shown if the user has a completed assessment');
      }
    }
  ...
  public function evaluate() {
    ...
    foreach ($assessments_codes as $item) {
      if ($item->status === '1' && !empty($item->results)) {
        $show = TRUE;
        break;
      }
    }
