...
abstract class FieldStateBase extends PluginBase implements FieldStateInterface, ContainerFactoryPluginInterface {
  ...
  public function applyState(array &$states, FormStateInterface $form_state, array $context, array $element, array $parents = NULL): bool {
    ...
    if ($form_object instanceof ContentEntityFormInterface) {
      ...
      $field_type = $target_field ? $target_field['type'] : NULL;
    }
    elseif (isset($context['items']) && $context['items'] instanceof FieldItemListInterface) {
      $field_type = $context['items']->getFieldDefinition()->getType();

      // When the parents are empty, try to fetch parents from the context.
      if (empty($parents) && isset($context['form']['#parents'])) {
