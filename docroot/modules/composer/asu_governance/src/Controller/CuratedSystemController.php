...
class CuratedSystemController extends SystemController {
  ...
  public function themesPage() :array {
    ...
    uasort($displayedThemes, [ThemeExtensionList::class, 'sortByName']);
    ...
    foreach ($displayedThemes as &$theme) {
      ...
      if (isset($displayedThemes[$theme->getName()]->base_themes)) {
        $theme_keys = array_keys($displayedThemes[$theme->getName()]->base_themes);
      ...
      foreach (array_reverse($theme_keys) as $theme_key) {
        if (isset($displayedThemes[$theme_key]) && file_exists($displayedThemes[$theme_key]->info['screenshot'])) {
          $theme->screenshot = [
            'uri' => $displayedThemes[$theme_key]->info['screenshot'],
      ...
      if (!empty($theme->status) || !$theme->info['core_incompatible'] && !$theme->incompatible_php && !$theme->incompatible_base && !$theme->incompatible_engine && !$theme->incompatible_module && empty($theme->module_dependencies_disabled)) {
        ...
        if ($this->themeAccess->checkAccess($theme->getName())) {
          $theme->operations[] = [
            ...
            'url' => Url::fromRoute('asu_governance.theme_settings_theme', ['theme' => $theme->getName()]),
        ...
        if (!empty($theme->status)) {
          if (!$theme->is_default) {
            ...
            foreach ($theme->required_by as $themename => $dependency) {
              if (!empty($displayedThemes[$themename]->status)) {
                $theme_uninstallable = FALSE;
              }
            }
            if ($theme_uninstallable) {
              $theme->operations[] = [
                ...
                'url' => Url::fromRoute('asu_governance.theme_uninstall'),
                'query' => $query,
            ...
            $theme->operations[] = [
              ...
              'url' => Url::fromRoute('asu_governance.theme_set_default'),
              'query' => $query,
          ...
          $theme->operations[] = [
            ...
            'url' => Url::fromRoute('asu_governance.theme_install'),
            'query' => $query,
